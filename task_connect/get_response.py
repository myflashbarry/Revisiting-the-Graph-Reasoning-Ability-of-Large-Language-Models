from task_connect.graph2nlp import get_single_data
import json
...
def get_response(folder_name, model, dataset_name, comment='', describe_method='node', prompt_method='zero', rename='', prompt_path='', seed=None, verbose=True, cover=False ):
    '''
    get LLM response of connectivity task and record into json
    json format: [{'text': <question text as string>, 'ans': <nx.has_path()>, 'response':[[[<response as string>], 0]]}]
    json_name = f"{model}{'_'.join(dataset_name.split('_')[:-1])}_connect{comment}_{describe_method}_{prompt_method}{'' if rename == '' else '-'+rename}"
    ...
    prompt_method: 'zero', 'zeropath', 'fewshot', 'zerocot', 'cot', for 'fewshot' and 'cot' please make sure that shot prompts are generated
