...
def judge_response(res_txt: str, json_path = '', question_txt = ''):
    match_yes = r'(^|\.|\s|\n|\'|\"|:)[Yy]es($|\.|,|\s|\n|\'|\")'
    match_no = r'(^|\.|\s|\n|\'|\"|:)[Nn]o($|\.|,|\s|\n|\'|\")'
    matchlist_yes = list(re.finditer(match_yes, res_txt))
    matchlist_no = list(re.finditer(match_no, res_txt))
    if matchlist_yes and not matchlist_no:
        llm_ans = 1
    elif matchlist_no and not matchlist_yes:
        llm_ans = 0
    elif matchlist_yes and matchlist_no:
        matchclass_yes = max(matchlist_yes, key=lambda x: x.end())
        matchclass_no = max(matchlist_no, key=lambda x: x.end())
        if matchclass_yes.end() > matchclass_no.end():
            llm_ans = 1
        else:
            llm_ans = 0
    else:
        ...
        # print(res_txt)
        # print(len(res_txt))
        # llm_ans = input(f'[Judge, input 0/1: {json_path}]')
        # print()
    return llm_ans

def get_acc(json_path, verbose=True):
    '''
    evaluate accuracy of connectivity question.
    ...
    Returns
    accuracy: ACC, simple judgment of right or wrong
    ----------
    '''
    response = load_json(json_path)
    total = 0
    y_true = []
    y_pred = []
    for res in response:
        if verbose:
            ...
            print(f"[ans] {res['ans']}")
        for res_txt in res['response'][0][0]:
            llm_ans = judge_response(res_txt, json_path, res['text'])
            if verbose:
                print(f'[response] {res_txt}')
                print(f'[judge] {llm_ans}')

            total += 1
            y_true.append(bool(res['ans']))
            y_pred.append(bool(llm_ans))
...
def judge_path(res, G, nodepair, idx, json_path, verbose):
    if not check_complete_res(res['response']):
    ...
    if verbose:
        print('[match nopath]', match_nopath(sentence[-100:]))
    ...
    if match_nopath(sentence[-100:]):
        ...
        if ans==0:
            ...
            shortest_path_length=nx.shortest_path_length(G,int(nodepair[0]),int(nodepair[1]))
            ...
            if path_verify:
                if extracted_length == shortest_path_length:
                    ...
                    return 1, 1, (shortest_path_length/extracted_length if extracted_length > 0 else 0)
            else:
                return 1, 0, -1


def get_acc_path(json_path, pkl_path, verbose=True):
    '''
    evaluate accuracy, F-acc and pcr of connectivity task, has to be answered with path.
    ...
    Returns
    accuracy: ACC, simple judgment of right or wrong without considering the path
    fidelity: F-acc, judge to be right only if the path is valid or truly no path, 
    ...
    for idx,(res,G,nodes) in enumerate(zip(data,graph_list,node_list)):
        try:
            acc, fed, pcr = judge_path(res=res, G=G, nodepair=nodes, idx=idx, json_path=json_path, verbose=verbose)
            if verbose:
                print('[acc]: ', acc, ', [fed]: ', fed, ', [pcr]: ', pcr)
